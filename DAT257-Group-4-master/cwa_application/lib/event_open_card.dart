...
class EventOpenCardState extends State<EventOpenCard> {
      ...
      builder: (BuildContext context) {
        ...
        progressDialog.style(
          ...
          progressWidget: CircularProgressIndicator(
              valueColor: AlwaysStoppedAnimation<Color>(Color(0xFF6740FB))),
          backgroundColor: Color(0xFF121212),
          messageTextStyle:
              TextStyle(color: Color(0xFFDCDCDC), fontFamily: 'Ginto'),
        );
              ...
              child: FlatButton(
                  ...
                  onPressed: addTicket
                      ...
                      : () async {
                          Navigator.of(context).pop();
                          progressDialog.show();
                          bool deleted = await httpService.deleteTicket2(
                              event.id, memberId);
                          if (deleted) {
                            if (this.mounted) {
                              setState(() {});
  ...
  CircularProgressIndicator progressIndicator() {
    return CircularProgressIndicator(
        valueColor: new AlwaysStoppedAnimation<Color>(Color(0xFF6740FB)));
  }
      ...
      child: Scaffold(
        ...
        body: CustomScrollView(
          slivers: <Widget>[
            SliverAppBar(
              pinned: true,
              backgroundColor: Color(0xFF6740FB),
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData &&
                ...
                if (ticketStatus == TicketStatus.available) {
                  return SafeArea(
                      child: attendButton("Get ticket", Color(0xFF6740FB)));
                } else if (ticketStatus == TicketStatus.event_full) {
                  return SafeArea(
                      child: attendButton(
                          "Event full", Colors.black.withOpacity(0.9)));
                } else if (ticketStatus == TicketStatus.event_outdated) {
                  return SafeArea(
                      child: attendButton(
                          "Event outdated", Colors.black.withOpacity(0.9)));
                } else if (ticketStatus == TicketStatus.owned_throwable) {
                  return SafeArea(
                      child: attendButton("Delete ticket", Colors.red));
                } else if (ticketStatus == TicketStatus.owned_not_throwable) {
                  return SafeArea(
                      child: attendButton(
                          "Less than 24h left", Colors.black.withOpacity(0.9)));
                }
              } else if(snapshot.hasError){
                return Text("Error getting ticketstatus from server", style: TextStyle(fontFamily: 'Ginto', fontSize: 22, color: Colors.red));
              }
